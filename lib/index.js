...
function getEnvVar(name, variables, defaultValue) {
  const split1 = name.split('{')
  const split1res = split1[2]
  const split2 = split1res.split('}')
  return getVarValue(variables, split2[0].toString(), defaultValue)
}
...
async function postmanToOpenApi (postmanJson, output, {
  ...
  for (let [i, element] of items.entries()) {
    ...
    let { path, query, protocol, host, port } = scrapeURL(url)

    if (host && host.length > 0 && host[0].split('{')[1] !== undefined) host = getEnvVar(host[0], variable, 'localhost')
    if (protocol && protocol.split('{')[1] !== undefined) protocol = getEnvVar(protocol, variable, 'http')
    if (port && port.split('{')[1] !== undefined) port = getEnvVar(port, variable, '80')
...
function calculateDomains (protocol, host, port) {
  if (protocol) return protocol + '://' + host + (port ? `:${port}` : '')
  return host
}
